    ...
    <div className="flex flex-col gap-3 p-4 rounded-2xl border"
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1">
          {[1,2,3,4,5].map((i) => (
            <StarFilled key={i} color={i <= review.rating ? "var(--gold)" : "#1f1f1f"} />
          ))}
          <span className="ml-1.5 text-[13px] font-bold" style={{ color: "var(--gold)" }}>
            {review.rating}.0
          </span>
        </div>
      ...
      {review.adminNote && (
        <div className="px-3 py-2 rounded-xl"
          ...
          <p className="text-[12.5px] leading-relaxed" style={{ color: "var(--text-secondary)" }}>{review.adminNote}</p>
        </div>
      )}
      <p className="text-[12px]" style={{ color: "var(--text-muted)" }}>{review.authorMasked}</p>
    </div>
  );
}
  ...
  return (
    <div className="min-h-dvh pb-24" style={{ background: "var(--bg)" }}>
      ...
      <header className="sticky top-0 z-40 px-4 pt-5 pb-4"
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-[20px] font-bold tracking-tight" style={{ color: "var(--text-primary)" }}>내 후기</h1>
          <div className="flex items-center gap-1.5">
            <StarFilled />
            <span className="text-[15px] font-bold" style={{ color: "var(--gold)" }}>{avgRating}</span>
            <span className="text-[12px]" style={{ color: "var(--text-dim)" }}>({reviews.length}개)</span>
          </div>
        </div>
      </header>
