...
import { getTrainerById, updateTrainer } from "@/app/data/trainers";
...
export default function TrainerEditPage({
  ...
  if (!loaded) return (
    <div className="min-h-dvh animate-pulse" style={{ background: "#0e0e0e" }}>
      <div className="h-14 mx-4 mt-4 rounded-2xl" style={{ background: "#1a1a1a" }} />
      <div className="flex flex-col gap-3 px-4 mt-6">
        {[1,2,3,4].map((i) => <div key={i} className="h-12 rounded-xl" style={{ background: "#1a1a1a" }} />)}
      </div>
    </div>
  );
      ...
      <div className="flex flex-col gap-px pb-36">
        ...
        <div className="px-4 pt-5 pb-4">
          ...
          {certs.length > 0 ? (
            ...
            <p className="text-[12.5px] text-center py-3" style={{ color: "rgba(255,255,255,0.06)" }}>
              추가된 항목이 없습니다.
            </p>
          )}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[12px]" style={{ color: "#5a5a5a" }}>
                {active ? "현재 회원에게 노출 중입니다." : "현재 숨겨진 상태입니다."}
              </p>
            </div>
            {/* 토글 스위치 */}
            <button
