...
function SearchIcon() {
  return (
    <svg width="19" height="19" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M16.5 16.5L21 21" stroke="var(--text-muted)" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  );
}
        ...
        {searchOpen ? (
          <div className="flex items-center gap-2 px-4 pt-5 pb-3">
            <div className="flex-1 flex items-center gap-2 px-3.5 py-2.5 rounded-2xl"
              style={{ background: "var(--surface)", border: "1.5px solid rgba(47,107,255,0.35)" }}>
              <SearchIcon />
              <input
                autoFocus
                value={searchQuery}
            ...
            <button onClick={closeSearch} className="text-[13px] font-medium" style={{ color: "var(--accent)" }}>
              취소
            </button>
          </div>
        ) : (
          <>
            ...
            <p className="px-4 text-[13px] pb-3" style={{ color: "var(--text-muted)" }}>
              나에게 맞는 트레이너를 찾아보세요.
            </p>
          </>
        )}
      ...
      <main className="page-scroll">
        {filtered.length > 0 ? (
                ...
                <div className="flex gap-1.5">
                  {[0, 1, 2].map((i) => (
                    <div key={i} className="w-1.5 h-1.5 rounded-full animate-pulse"
                      style={{ background: "var(--text-dim)", animationDelay: `${i * 150}ms` }} />
                  ))}
                </div>
              </div>
            ) : (
              <div className="text-center pt-3 pb-6">
                <p className="text-[12px]" style={{ color: "var(--text-dim)" }}>모든 트레이너를 확인했습니다.</p>
                <button onClick={() => setSelectedCategory("all")} className="mt-1.5 text-[12px] font-medium" style={{ color: "var(--accent)" }}>
                  전체 트레이너 다시 보기
                </button>
              </div>
            )}
          ...
          <div className="flex flex-col items-center justify-center pt-28 px-8 text-center">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4" style={{ background: "var(--accent-glow)" }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" stroke="var(--accent)" strokeWidth="1.8" strokeLinecap="round" />
                <circle cx="9" cy="7" r="4" stroke="var(--accent)" strokeWidth="1.8" />
                <path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" stroke="var(--accent)" strokeWidth="1.8" strokeLinecap="round" />
              </svg>
            </div>
